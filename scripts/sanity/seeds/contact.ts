/**
 * scripts/sanity/seeds/contact.ts
 *
 * Seeds the Sanity contactPageSettings singleton for the /contact page.
 * Called by the orchestrator via `npm run sanity:seed`.
 * Idempotent — uses createOrReplace, safe to run multiple times.
 */

import { client } from '../lib';
import type { SanityContactPageSettingsDoc } from '../../types';

async function seedContactPageSettings() {
  console.log('\n── Contact › page settings ──');

  const doc: SanityContactPageSettingsDoc = {
  ...
  console.log(`  ✓ contactPageSettings upserted (_id: ${result._id})`);
}

export async function seedContact() {
  console.log('\nSeeding Sanity — Contact page');
