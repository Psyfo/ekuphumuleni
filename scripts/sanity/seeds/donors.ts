/**
 * scripts/sanity/seeds/donors.ts
 *
 * Seeds the Sanity donorsPageSettings singleton and donorYear documents
 * for the /donors page. Called by the orchestrator via `npm run sanity:seed`.
 * Idempotent — uses createOrReplace, safe to run multiple times.
 */

import { client } from '../lib';
import type { SanityDonorsPageSettingsDoc, SanityDonorYearDoc } from '../../types';

async function seedDonorsPageSettings() {
  console.log('\n── Donors › page settings ──');

  const doc: SanityDonorsPageSettingsDoc = {
  ...
  console.log(`  ✓ donorsPageSettings upserted (_id: ${result._id})`);
}

async function seedDonorYears() {
  console.log('\n── Donors › donor years ──');

  const years: SanityDonorYearDoc[] = [
    {
      _id: 'donorYear-2003',
      _type: 'donorYear',
      year: 2003,
  ...
  for (const doc of years) {
    const result = await client.createOrReplace(doc);
    console.log(`  ✓ donorYear ${doc.year} upserted (_id: ${result._id})`);
  }
}

export async function seedDonors() {
  console.log('\nSeeding Sanity — Donors page');
  await seedDonorsPageSettings();
  await seedDonorYears();
}
